...
def docs_management_menu(api, workspace_id):
...
def main():
    ...
    # Get workspace (auto-select if only one)
    try:
        teams = api.get("team")
        workspace = teams['teams'][0]  # Auto-select first workspace
        print(f"✓ Using workspace: {workspace['name']}")
    except Exception as e:
        print(f"\n✗ Failed to get workspace: {e}")
        return
    ...
    while True:
        ...
        if choice == "0":
            ...
            docs_management_menu(api, workspace['id'])
